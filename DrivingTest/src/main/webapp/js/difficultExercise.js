...
function showQuiz(data) {
	...
	for (var i = 0; i < answers.length - 1; i++) {
		str += '<p  data-answer="16" onclick="choseAnswer(\'' + answers[i]
				+ '\',' + i + ',' + data.qid + ',' + data.errTotal
				+ ')" class="choseP">';
		str += '<i id="optionImg' + i
				+ '" ></i><span style="font-size: 18px;">' + answers[i]
				+ '</span>';
		str += '</p>';
	}
	var type;
	...
	if ($.cookie('recordAnswer') != undefined
		...
		if ($.cookie('recordAnswer').indexOf("@" + data.qid + ",") != -1) {
			...
			for (var i = 1; i < recordAnswers.length; i++) {
				 ...
				 * ));
				 */if (recordAnswers[i].substring(0, recordAnswers[i]
						.indexOf(",")) == data.qid) {
...
function choseAnswer(answer, i, qid, errTotal) {
	...
	if (recordAnswer.indexOf(("@" + qid + "," + answer)) == -1) {
		...
		$.cookie('recordAnswer', recordAnswer, {
			path : '/'
		});
	}
	if (right == answer) {
		$("#optionImg" + i).css({
			"background-image" : "url(images/optionRight.png)"
		});
		var type = '<p style="color:#2da5ec;font-size: 18px;">回答正确!</p>';
		$("#qtype").html(type);
		if (cid > 0 && flag) {
			$.post("selfErrors/removeErrorSave", {
				"cid" : cid,
				"errorSave" : qid
			}, function(data) {
				if (data) {
		...
		$("#optionImg" + i).css({
			"background-image" : "url(images/optionError.png)"
		});
		var type = '<p style="font-size: 18px;"><label style="color:red;">回答错误!&nbsp;&nbsp;&nbsp;&nbsp;</label>正确答案: <b style="color:#2da5ec;">'
				+ right + '</b></p>';
		$("#qtype").html(type);
		switch (right) {
		case 'A':
		...
		var errorSave = qid;
		if (cid > 0 && flag) {
			$.post("selfErrors/saveSelfErrors", {
				"cid" : cid,
				"errorSave" : errorSave
			}, function(data) {
				if (data) {
...
function showFavorSave(qid) {
	var cid = $("#cid").text();
	if (cid > 0) {
		$.post("selfErrors/getFavorSave", {
			"cid" : cid
		}, function(data) {
			var saves = data.save;
			if (saves.indexOf(qid)!=-1) {
				$("#favor-tag").css({
					"background-image" : "url(images/fovor1.png)"
				});
			} else {
				$("#favor-tag").css({
					"background-image" : "url(images/fovor.png)"
				});
			}
